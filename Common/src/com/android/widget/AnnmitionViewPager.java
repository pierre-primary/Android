...
public class AnnmitionViewPager extends ViewPager {
    private  boolean isFirst = true;
	public static final String TAG = "AnnmitionViewPager";

	private boolean mEnabled = true;
	private boolean mFadeEnabled = false;
	private boolean mOutlineEnabled = false;
	public static int sOutlineColor = Color.WHITE;
	private TransitionEffect mEffect = TransitionEffect.FlipHorizontal;
    ...
    public AnnmitionViewPager(Context context) {
		this(context, null);
	}

	@SuppressWarnings("incomplete-switch")
	public AnnmitionViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		setClipChildren(false);
		// now style everything!
		TypedArray ta = context.obtainStyledAttributes(attrs, com.android.common.R.styleable.AnnmitionViewPager);
		int effect = ta.getInt(com.android.common.R.styleable.AnnmitionViewPager_style, 0);
		String[] transitions = getResources().getStringArray(com.android.common.R.array.jazzy_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(com.android.common.R.styleable.AnnmitionViewPager_fadeEnabled, false));
		setOutlineEnabled(ta.getBoolean(com.android.common.R.styleable.AnnmitionViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(com.android.common.R.styleable.AnnmitionViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
			setFadeEnabled(true);
...
//		reset();
    ...
    public TransitionEffect getmEffect() {
        return mEffect;
    }
    ...
    private void setViewPagerScroller() {
        try {
            ...
            scroller.setScrollDurationFactor(6.0);
...
//                    logState(left,"left");
					...
					ViewHelper.setPivotX(left, left.getMeasuredWidth() / 2.0f);
					ViewHelper.setPivotY(left, left.getMeasuredHeight() / 2.0f);
					ViewHelper.setTranslationX(left, mTrans);
					ViewHelper.setRotationY(left, mRot);
//                    logState(left,"left");
				}
...
//                    logState(right,"right");
					...
					ViewHelper.setPivotX(right, right.getMeasuredWidth() / 2.0f);
					ViewHelper.setPivotY(right, right.getMeasuredHeight() / 2.0f);
					ViewHelper.setTranslationX(right, mTrans);
					ViewHelper.setRotationY(right, mRot);
//                    logState(right,"right");
				}
