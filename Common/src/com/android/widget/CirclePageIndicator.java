...
/**
 * Created by xudong on 2015/4/23.
 */
...
public class CirclePageIndicator extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, com.android.common.R.styleable.CirclePageIndicator, 0, 0);
        try {
            indicatorSpacing = a.getDimensionPixelSize(
                    com.android.common.R.styleable.CirclePageIndicator_indicator_spacing, DEFAULT_INDICATOR_SPACING);
        } finally {
            a.recycle();
        }
    ...
    private void addIndicator(int count) {
        for (int i = 0; i < count; i++) {
            ...
            img.setImageResource(com.android.common.R.drawable.circle_indicator_stroke);
            addView(img, params);
        }

        if (count > 0) {
            ((ImageView) getChildAt(0)).setImageResource(com.android.common.R.drawable.circle_indicator_solid);
        }
    }

    private void updateIndicator(int position) {
        if (activePosition != position) {
            ((ImageView) getChildAt(activePosition)).setImageResource(com.android.common.R.drawable.circle_indicator_stroke);
            ((ImageView) getChildAt(position)).setImageResource(com.android.common.R.drawable.circle_indicator_solid);
